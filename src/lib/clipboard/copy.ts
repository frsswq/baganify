import { shapesToSVGDocument, getShapesBoundingBox } from '../shapes/renderer';
 ...
 */
export async function copyShapesToClipboard(shapes: Shape[]): Promise<{ success: boolean; format: string }> {
  if (shapes.length === 0) {
 ...
 */
async function svgToPng(svgContent: string, width: number, height: number): Promise<Blob> {
